...
// --- Visual Coupon Editor ---
function VisualCouponEditor({ coupons, onChange }: { coupons: Coupon[]; onChange: (updated: Coupon[]) => void }) {
  ...
  return (
    <div className="flex flex-col gap-3 border-4 border-black rounded-2xl p-4 bg-[#FFF8F0]">
      ...
      {showJsonMode ? (
        ...
        <>
          ...
          {(!coupons || coupons.length === 0) ? (
            <div className="text-center py-4 text-xs font-bold text-stone-400 border-2 border-dashed border-stone-300 rounded-xl">
              Belum ada voucher. Klik "+ Tambah Voucher" di atas.
            </div>
          ) : (
            <div className="flex flex-col gap-2">
              {coupons.map(c => {
                const titleText = typeof c.title === 'string' ? c.title : (c.title?.id || c.title?.en || c.title?.ko || '');
                return (
...
function PartnerForm({ initialData, onSave, categories }: { initialData: any, onSave: (data: any) => void, categories: Category[] }) {
  ...
  const [formData, setFormData] = useState(() => {
    ...
    if (!base.description) base.description = { ko: '', en: '', id: '' };
    return base;
  });
  ...
  const handleDescChange = (lang: string, val: string) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
         ...
         <div className="flex flex-col gap-1">
           ...
           <select value={formData.categoryId} onChange={e => handleChange('categoryId', e.target.value)} className="border-4 border-black rounded-xl p-3 font-bold bg-[#FFF8F0]">
             {categories.map(c => (
               <option key={c.id} value={c.id}>{c.name?.en || c.name?.ko || c.id}</option>
             ))}
           </select>
         </div>
       </div>
       ...
       <div className="flex flex-col gap-1 bg-[#FFF8F0] border-4 border-black rounded-2xl p-4">
         <div className="flex justify-between items-center mb-2 border-b-2 border-black/10 pb-2">
           <label className="font-black text-sm uppercase">Description Translation</label>
           <div className="flex gap-1">
             {(['en', 'ko', 'id'] as const).map(lang => (
               <button
                 key={lang}
                 type="button"
                 onClick={() => setLangTab(lang)}
                 className={`px-3 py-1 font-black text-xs uppercase rounded-lg border-2 border-black transition-all ${
                   langTab === lang ? 'bg-[#FDD835] shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]' : 'bg-white'
                 }`}
               >
                 {lang.toUpperCase()}
               </button>
         ...
         <textarea
           ...
           onChange={e => handleDescChange(langTab, e.target.value)}
           placeholder={`Enter description in ${langTab.toUpperCase()}...`}
           className="border-4 border-black rounded-xl p-3 font-bold bg-white focus:outline-none"
         />
       </div>
       ...
       <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
         <div className="flex flex-col gap-1">
           ...
           <div className="flex gap-2">
             <input type="text" value={formData.logo} onChange={e => handleChange('logo', e.target.value)} className="border-4 border-black rounded-xl p-3 font-bold bg-[#FFF8F0] flex-1 text-sm" />
             <label className="bg-[#FDD835] border-4 border-black px-3 py-2 rounded-xl font-black text-xs uppercase cursor-pointer flex items-center shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:shadow-none">
               Upload
       ...
       <div className="flex flex-col gap-1 border-t-2 border-black/10 pt-2">
         <label className="font-black text-sm uppercase">🔗 Link Menu Digital (PDF, Google Drive, Canva, dll.)</label>
         <input
           type="url"
           value={formData.menuUrl || ''}
           onChange={e => handleChange('menuUrl', e.target.value)}
           placeholder="https://bit.ly/menu-partner-saya"
           className="border-4 border-black rounded-xl p-3 font-bold text-sm bg-[#FFF8F0]"
         />
       </div>

       <VisualCouponEditor coupons={formData.coupons || []} onChange={updated => handleChange('coupons', updated)} />
